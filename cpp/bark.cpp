...
struct gpt_layer {
    // normalization
    struct bark_ggml_tensor* ln_1_g;
    struct bark_ggml_tensor* ln_1_b;

    struct bark_ggml_tensor* ln_2_g;
    struct bark_ggml_tensor* ln_2_b;

    // attention
    struct bark_ggml_tensor* c_attn_attn_w;
    struct bark_ggml_tensor* c_attn_attn_b;

    struct bark_ggml_tensor* c_attn_proj_w;
    struct bark_ggml_tensor* c_attn_proj_b;

    // mlp
    struct bark_ggml_tensor* c_mlp_fc_w;
    struct bark_ggml_tensor* c_mlp_fc_b;

    struct bark_ggml_tensor* c_mlp_proj_w;
    struct bark_ggml_tensor* c_mlp_proj_b;
};
...
struct gpt_model {
    ...
    // normalization
    struct bark_ggml_tensor* ln_f_g;
    struct bark_ggml_tensor* ln_f_b;

    struct bark_ggml_tensor* wpe;                    //  position embedding
    std::vector<struct bark_ggml_tensor*> wtes;      //     token embedding
    std::vector<struct bark_ggml_tensor*> lm_heads;  // language model head

    std::vector<gpt_layer> layers;

    // key + value memory
    struct bark_ggml_tensor* memory_k;
    struct bark_ggml_tensor* memory_v;
    ...
    bark_ggml_backend_buffer_t buffer_kv;
...
static bark_token gpt_sample(
    ...
    int64_t* t_sample_us,
    int64_t* n_sample) {
    int64_t t_sample_start_us = bark_ggml_time_us();
    ...
    int64_t t_sample_end_us = bark_ggml_time_us();
...
static bool bark_ggml_quantize_weights(
    std::ifstream& fin,
    std::ofstream& fout,
    const bark_ggml_ftype ftype,
    const std::vector<std::string>& to_quant,
    const std::vector<std::string>& to_skip) {
    bark_ggml_type qtype = BARK_GGML_TYPE_F32;

    switch (ftype) {
        case BARK_GGML_FTYPE_MOSTLY_Q4_0:
            qtype = BARK_GGML_TYPE_Q4_0;
            break;
        case BARK_GGML_FTYPE_MOSTLY_Q4_1:
            qtype = BARK_GGML_TYPE_Q4_1;
            break;
        case BARK_GGML_FTYPE_MOSTLY_Q5_0:
            qtype = BARK_GGML_TYPE_Q5_0;
            break;
        case BARK_GGML_FTYPE_MOSTLY_Q5_1:
            qtype = BARK_GGML_TYPE_Q5_1;
            break;
        case BARK_GGML_FTYPE_MOSTLY_Q8_0:
            qtype = BARK_GGML_TYPE_Q8_0;
            break;
        case BARK_GGML_FTYPE_UNKNOWN:
        case BARK_GGML_FTYPE_ALL_F32:
        case BARK_GGML_FTYPE_MOSTLY_F16:
        case BARK_GGML_FTYPE_MOSTLY_Q4_1_SOME_F16:
        case BARK_GGML_FTYPE_MOSTLY_Q2_K:
        case BARK_GGML_FTYPE_MOSTLY_Q3_K:
        case BARK_GGML_FTYPE_MOSTLY_Q4_K:
        case BARK_GGML_FTYPE_MOSTLY_Q5_K:
        case BARK_GGML_FTYPE_MOSTLY_Q6_K: {
            LOGE("%s: invalid model type %d\n", __func__, ftype);
            return false;
        }
    };

    if (!bark_ggml_is_quantized(qtype)) {
        LOGE("%s: invalid quantization type %d (%s)\n", __func__, qtype, bark_ggml_type_name(qtype));
        return false;
    }
    ...
    std::vector<bark_ggml_fp16_t> data_f16;
    ...
    for (int i = 0; i < n_tensors; i++) {
        ...
        LOGI("%64s - [%5d, %5d, %5d], type = %6s ", name.data(), ne[0], ne[1], ne[2], bark_ggml_type_name((bark_ggml_type)ttype));
        ...
        if (quantize) {
            if (ttype != BARK_GGML_TYPE_F32 && ttype != BARK_GGML_TYPE_F16) {
                LOGE("%s: unsupported ttype %d (%s) for integer quantization\n", __func__, ttype, bark_ggml_type_name((bark_ggml_type)ttype));
                return false;
            }

            if (ttype == BARK_GGML_TYPE_F16) {
                data_f16.resize(nelements);
                fin.read(reinterpret_cast<char*>(data_f16.data()), nelements * sizeof(bark_ggml_fp16_t));
                data_f32.resize(nelements);
                for (int i = 0; i < nelements; ++i) {
                    data_f32[i] = bark_ggml_fp16_to_fp32(data_f16[i]);
                }
            } else {
                data_f32.resize(nelements);
        ...
        if (quantize) {
            ...
            switch ((bark_ggml_type)ttype) {
                case BARK_GGML_TYPE_Q4_0: {
                    cur_size = bark_ggml_quantize_q4_0(data_f32.data(), work.data(), nelements, ne[0], hist_cur.data());
                } break;
                case BARK_GGML_TYPE_Q4_1: {
                    cur_size = bark_ggml_quantize_q4_1(data_f32.data(), work.data(), nelements, ne[0], hist_cur.data());
                } break;
                case BARK_GGML_TYPE_Q5_0: {
                    cur_size = bark_ggml_quantize_q5_0(data_f32.data(), work.data(), nelements, ne[0], hist_cur.data());
                } break;
                case BARK_GGML_TYPE_Q5_1: {
                    cur_size = bark_ggml_quantize_q5_1(data_f32.data(), work.data(), nelements, ne[0], hist_cur.data());
                } break;
                case BARK_GGML_TYPE_Q8_0: {
                    cur_size = bark_ggml_quantize_q8_0(data_f32.data(), work.data(), nelements, ne[0], hist_cur.data());
                } break;
                ...
                case BARK_GGML_TYPE_COUNT: {
                    LOGE("%s: unsupported quantization type %d (%s)\n", __func__, ttype, bark_ggml_type_name((bark_ggml_type)ttype));
                    return false;
                }
            }
            ...
            LOGI("size = %8.2f MB -> %8.2f MB | hist: ", nelements * sizeof(float) / 1024.0 / 1024.0, cur_size / 1024.0 / 1024.0);
            for (int i = 0; i < (int)hist_cur.size(); ++i) {
                hist_all[i] += hist_cur[i];
            }

            for (int i = 0; i < (int)hist_cur.size(); ++i) {
                LOGI("%5.3f ", hist_cur[i] / (float)nelements);
            }
            LOGI("\n");
        } else {
    ...
    LOGI("%s: quant size  = %8.2f MB | ftype = %d (%s)\n", __func__, total_size_new / 1024.0 / 1024.0, ftype, bark_ggml_type_name(qtype));

    {
        ...
        LOGI("%s: hist: ", __func__);
        for (int i = 0; i < (int)hist_all.size(); ++i) {
            LOGI("%5.3f ", hist_all[i] / (float)sum_all);
        }
        LOGI("\n");
    }
...
static void bark_tokenize_input(struct bark_context* bctx, const std::string& text) {
    ...
    if (n_tokens < max_ctx_size) {
        ...
        LOGE("%s: input sequence is too long (%d > 256), truncating sequence", __func__, n_tokens);
    }
    ...
    LOGI("%s: prompt: '%s'\n", __func__, text.c_str());
    LOGI("%s: number of tokens in prompt = %zu, first 8 tokens: ", __func__, bctx->tokens.size());
    for (int i = 0; i < std::min(8, (int)bctx->tokens.size()); i++) {
        LOGI("%d ", bctx->tokens[i]);
    }
    LOGI("\n\n");
}
...
static bool bark_model_load(std::ifstream& fin, gpt_model& model, int n_gpu_layers, bark_verbosity_level verbosity) {
    ...
    // Evaluating context size
    {
        ...
        buffer_size += n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32);  // ln_f_g

        buffer_size += n_wtes * n_in_vocab * n_embd * bark_ggml_type_size(wtype);       // wtes
        buffer_size += block_size * n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32);        // wpe
        buffer_size += n_lm_heads * n_out_vocab * n_embd * bark_ggml_type_size(wtype);  // lm_head

        buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // ln_1_g
        buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // ln_2_g

        buffer_size += n_layer * (3 * n_embd * n_embd * bark_ggml_type_size(wtype));  // c_attn_attn_w
        buffer_size += n_layer * (n_embd * n_embd * bark_ggml_type_size(wtype));      // c_attn_proj_w

        buffer_size += n_layer * (4 * n_embd * n_embd * bark_ggml_type_size(wtype));  // c_mlp_fc_w
        buffer_size += n_layer * (4 * n_embd * n_embd * bark_ggml_type_size(wtype));  // c_mlp_proj_w

        if (bias) {
            buffer_size += n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32);  // ln_f_b

            buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // ln_1_b
            buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // ln_2_b

            buffer_size += n_layer * (3 * n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // c_attn_attn_b
            buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));      // c_attn_proj_b

            buffer_size += n_layer * (4 * n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));  // c_mlp_fc_b
            buffer_size += n_layer * (n_embd * bark_ggml_type_size(BARK_GGML_TYPE_F32));      // c_mlp_proj_b
        }
    ...
    if (!model.backend) {
        ...
        if (verbosity == bark_verbosity_level::HIGH) {
            LOGE("%s: no backend specified, using CPU backend\n", __func__);
        }
    ...
    if (!model.backend) {
        if (verbosity == bark_verbosity_level::HIGH) {
            LOGE("%s: failed to initialize CPU backend\n", __func__);
        }
    ...
    // prepare memory for the weights
    {
        ...
        model.ln_f_g = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);

        if (bias) {
            model.ln_f_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);
        }

        model.wpe = bark_ggml_new_tensor_2d(ctx, BARK_GGML_TYPE_F32, n_embd, block_size);

        for (int i = 0; i < n_wtes; i++) {
            model.wtes[i] = bark_ggml_new_tensor_2d(ctx, wtype, n_embd, n_in_vocab);
            model.tensors["model/wte/" + std::to_string(i)] = model.wtes[i];
        }

        for (int i = 0; i < n_lm_heads; i++) {
            model.lm_heads[i] = bark_ggml_new_tensor_2d(ctx, wtype, n_embd, n_out_vocab);
        ...
        for (int i = 0; i < n_layer; ++i) {
            ...
            layer.ln_1_g = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);
            layer.ln_2_g = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);

            layer.c_attn_attn_w = bark_ggml_new_tensor_2d(ctx, wtype, n_embd, 3 * n_embd);
            layer.c_attn_proj_w = bark_ggml_new_tensor_2d(ctx, wtype, n_embd, n_embd);

            layer.c_mlp_fc_w = bark_ggml_new_tensor_2d(ctx, wtype, n_embd, 4 * n_embd);
            layer.c_mlp_proj_w = bark_ggml_new_tensor_2d(ctx, wtype, 4 * n_embd, n_embd);

            if (bias) {
                layer.ln_1_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);
                layer.ln_2_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);

                layer.c_attn_attn_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, 3 * n_embd);
                layer.c_attn_proj_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);

                layer.c_mlp_fc_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, 4 * n_embd);
                layer.c_mlp_proj_b = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_embd);
            }
    ...
    // key + value memory
    {
        ...
        if (n_lm_heads == 1 && n_wtes == 1) {
            ...
            model.memory_k = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_elements);
            model.memory_v = bark_ggml_new_tensor_1d(ctx, BARK_GGML_TYPE_F32, n_elements);

            const size_t memory_size = bark_ggml_nbytes(model.memory_k) + bark_ggml_nbytes(model.memory_v);

            if (verbosity == bark_verbosity_level::HIGH) {
                LOGI("%s: memory size = %8.2f MB, n_mem = %d\n", __func__, memory_size / 1024.0 / 1024.0, n_mem);
            }

            // create a backend buffer (can be in host or device memory)
            model.buffer_kv = bark_ggml_backend_alloc_buffer(model.backend, memory_size + 256);

            // allocate the tensors into the backend buffer
            {
                bark_ggml_allocr* alloc = bark_ggml_allocr_new_from_buffer(model.buffer_kv);

                // this updates the pointers in the tensors to point to the correct location in the buffer
                // this is necessary since the bark_ggml_context is .no_alloc == true
                // note that the buffer can actually be a device buffer, depending on the backend
                bark_ggml_allocr_alloc(alloc, model.memory_k);
                bark_ggml_allocr_alloc(alloc, model.memory_v);
    ...
    // load weights
    {
        ...
        for (int i = 0; i < n_tensors; i++) {
            ...
            const size_t bpe = bark_ggml_type_size(bark_ggml_type(ttype));
            ...
            if (bark_ggml_backend_is_cpu(model.backend)
                ...
                fin.read(reinterpret_cast<char*>(tensor->data), bark_ggml_nbytes(tensor));
            } else {
                // read into a temporary buffer first, then copy to device memory
                read_buf.resize(bark_ggml_nbytes(tensor));
                fin.read(read_buf.data(), bark_ggml_nbytes(tensor));
                bark_ggml_backend_tensor_set(tensor, read_buf.data(), 0, bark_ggml_nbytes(tensor));
            }

            if (verbosity == bark_verbosity_level::HIGH) {
                LOGI("%48s - [%5d, %5d], type = %6s, %6.2f MB\n", name.data(), ne[0], ne[1], "float", bark_ggml_nbytes(tensor) / 1024.0 / 1024.0);
            }
...
static bool bark_load_model_from_file(
    ...
    // verify magic
    {
        ...
        if (magic != BARK_GGML_FILE_MAGIC) {
            LOGE("%s: invalid model file '%s' (bad magic)\n", __func__, fname.c_str());
            return false;
        }
    }
    ...
    {
        if (verbosity == bark_verbosity_level::MEDIUM || verbosity == bark_verbosity_level::HIGH) {
            LOGI("%s: reading bark vocab\n", __func__);
        }

        if (!bark_vocab_load(fin, &bctx->text_model.vocab)) {
            LOGE("%s: failed to load vocab\n", __func__);
            return false;
        }
    }
    ...
    // coarse
    {
        if (!bark_model_load(fin, bctx->text_model.coarse_model, n_gpu_layers, verbosity)) {
            LOGE("%s: invalid model file '%s' (bad coarse)\n", __func__, fname.c_str());
            return false;
        }
    }
    ...
    // fine
    {
        if (!bark_model_load(fin, bctx->text_model.fine_model, n_gpu_layers, verbosity)) {
            LOGE("%s: invalid model file '%s' (bad fine)\n", __func__, fname.c_str());
            return false;
        }
    }
    ...
    // codec model
    {
        ...
        if (!bctx->encodec_ctx) {
            LOGE("%s: invalid model file '%s' (bad encodec)\n", __func__, fname.c_str());
            return false;
        }
    }
...
struct bark_context* bark_load_model(const char* model_path, struct bark_context_params params, uint32_t seed) {
    int64_t t_load_start_us = bark_ggml_time_us();
    ...
    bctx->params = params;
    bctx->stats.t_load_us = bark_ggml_time_us() - t_load_start_us;

    return bctx;
}

static struct bark_ggml_cgraph* bark_build_gpt_graph(
    gpt_model* model,
    bark_ggml_allocr* allocr,
    bark_sequence& tokens,
    int* n_past,
    bool merge_ctx,
    int n_threads) {
    if (!n_past) {
        LOGE("%s: n_past is null\n", __func__);
        return NULL;
    }
    ...
    struct bark_ggml_tensor* input = bark_ggml_new_tensor_1d(ctx0, BARK_GGML_TYPE_I32, N);
    bark_ggml_allocr_alloc(allocr, input);

    // avoid writing to tensors if we are only measuring the memory usage
    if (!bark_ggml_allocr_is_measure(allocr)) {
        bark_ggml_backend_tensor_set(input, tokens.data(), 0, N * bark_ggml_element_size(input));
    }

    struct bark_ggml_tensor* tok_emb;

    if (*n_past > 0) {
        assert(N == 1);
        tok_emb = bark_ggml_get_rows(ctx0, model->wtes[0], input);
    } else {
        if (merge_ctx) {
            assert(N == 256 + 256 + 1);
            N -= 256;
        ...
        if (merge_ctx) {
            struct bark_ggml_tensor* seq_embd = bark_ggml_get_rows(ctx0, model->wtes[0], bark_ggml_view_1d(ctx0, input, 256, 0));
            struct bark_ggml_tensor* ctx_embd = bark_ggml_get_rows(ctx0, model->wtes[0], bark_ggml_view_1d(ctx0, input, 256, 256 * bark_ggml_element_size(input)));
            struct bark_ggml_tensor* rem_embd = bark_ggml_get_rows(ctx0, model->wtes[0], bark_ggml_view_1d(ctx0, input, 1, 512 * bark_ggml_element_size(input)));

            struct bark_ggml_tensor* cat_emb = bark_ggml_add(ctx0, seq_embd, ctx_embd);

            tok_emb = bark_ggml_new_tensor_2d(ctx0, cat_emb->type, cat_emb->ne[0], cat_emb->ne[1] + rem_embd->ne[1]);
            bark_ggml_allocr_alloc(allocr, tok_emb);

            tok_emb = bark_ggml_set_1d(ctx0, tok_emb, cat_emb, 0);
            tok_emb = bark_ggml_set_1d(ctx0, tok_emb, rem_embd, cat_emb->ne[0] * cat_emb->ne[1] * bark_ggml_element_size(cat_emb));
        } else {
            tok_emb = bark_ggml_get_rows(ctx0, model->wtes[0], input);
        }
    }

    struct bark_ggml_tensor* position = bark_ggml_new_tensor_1d(ctx0, BARK_GGML_TYPE_I32, N);
    bark_ggml_allocr_alloc(allocr, position);
    if (!bark_ggml_allocr_is_measure(allocr)) {
        for (int i = 0; i < N; ++i) {
            int32_t v = *n_past + i;
            bark_ggml_backend_tensor_set(position, &v, i * sizeof(int32_t), sizeof(v));
        }
    }

    struct bark_ggml_tensor* KQ_scale = bark_ggml_new_tensor_1d(ctx0, BARK_GGML_TYPE_F32, 1);
    bark_ggml_allocr_alloc(allocr, KQ_scale);
    if (!bark_ggml_allocr_is_measure(allocr)) {
        float s = 1.0f / sqrtf(float(n_embd) / n_head);
        bark_ggml_backend_tensor_set(KQ_scale, &s, 0, sizeof(s));
    }

    // wte + wpe
    struct bark_ggml_tensor* inpL = bark_ggml_add(ctx0, tok_emb, bark_ggml_get_rows(ctx0, model->wpe, position));

    for (int il = 0; il < n_layer; ++il) {
        struct bark_ggml_tensor* cur;

        // norm
        {
            cur = bark_ggml_norm(ctx0, inpL, EPS_NORM);

            // cur = ln_1_g*cur + ln_1_b
            cur = bark_ggml_mul(ctx0, cur, model->layers[il].ln_1_g);

            if (bias) {
                cur = bark_ggml_add(ctx0, cur, model->layers[il].ln_1_b);
            }
        }

        // attn
        {
            cur = bark_ggml_mul_mat(ctx0,
                               model->layers[il].c_attn_attn_w,
                               cur);

            if (bias) {
                cur = bark_ggml_add(ctx0, cur, model->layers[il].c_attn_attn_b);
            }
        }

        // self-attention
        {
            struct bark_ggml_tensor* Qcur = bark_ggml_view_2d(ctx0, cur, n_embd, N, cur->nb[1], 0 * sizeof(float) * n_embd);
            struct bark_ggml_tensor* Kcur = bark_ggml_view_2d(ctx0, cur, n_embd, N, cur->nb[1], 1 * sizeof(float) * n_embd);
            struct bark_ggml_tensor* Vcur = bark_ggml_view_2d(ctx0, cur, n_embd, N, cur->nb[1], 2 * sizeof(float) * n_embd);

            // store key and value to memory
            if (N >= 1) {
                struct bark_ggml_tensor* k = bark_ggml_view_1d(ctx0, model->memory_k, N * n_embd, (bark_ggml_element_size(model->memory_k) * n_embd) * (il * n_ctx + *n_past));
                struct bark_ggml_tensor* v = bark_ggml_view_1d(ctx0, model->memory_v, N * n_embd, (bark_ggml_element_size(model->memory_v) * n_embd) * (il * n_ctx + *n_past));

                bark_ggml_build_forward_expand(gf, bark_ggml_cpy(ctx0, Kcur, k));
                bark_ggml_build_forward_expand(gf, bark_ggml_cpy(ctx0, Vcur, v));
            }

            struct bark_ggml_tensor* Q =
                bark_ggml_permute(ctx0,
                             bark_ggml_cpy(ctx0,
                                      Qcur,
                                      bark_ggml_new_tensor_3d(ctx0, BARK_GGML_TYPE_F32, n_embd / n_head, n_head, N)),
                             0, 2, 1, 3);

            struct bark_ggml_tensor* K =
                bark_ggml_permute(ctx0,
                             bark_ggml_reshape_3d(ctx0,
                                             bark_ggml_view_1d(ctx0, model->memory_k, (*n_past + N) * n_embd, il * n_ctx * bark_ggml_element_size(model->memory_k) * n_embd),
                                             n_embd / n_head, n_head, *n_past + N),
                             0, 2, 1, 3);

            struct bark_ggml_tensor* KQ = bark_ggml_mul_mat(ctx0, K, Q);

            struct bark_ggml_tensor* KQ_scaled = bark_ggml_scale_inplace(ctx0, KQ, KQ_scale);

            struct bark_ggml_tensor* KQ_masked = bark_ggml_diag_mask_inf_inplace(ctx0, KQ_scaled, *n_past);

            struct bark_ggml_tensor* KQ_soft_max = bark_ggml_soft_max_inplace(ctx0, KQ_masked);

            struct bark_ggml_tensor* V_trans =
                bark_ggml_cpy(ctx0,
                         bark_ggml_permute(ctx0,
                                      bark_ggml_reshape_3d(ctx0,
                                                      bark_ggml_view_1d(ctx0, model->memory_v, (*n_past + N) * n_embd, il * n_ctx * bark_ggml_element_size(model->memory_v) * n_embd),
                                                      n_embd / n_head, n_head, *n_past + N),
                                      1, 2, 0, 3),
                         bark_ggml_new_tensor_3d(ctx0, model->memory_v->type, *n_past + N, n_embd / n_head, n_head));

            struct bark_ggml_tensor* KQV = bark_ggml_mul_mat(ctx0, V_trans, KQ_soft_max);

            struct bark_ggml_tensor* KQV_merged = bark_ggml_permute(ctx0, KQV, 0, 2, 1, 3);

            cur = bark_ggml_cpy(ctx0,
                           KQV_merged,
                           bark_ggml_new_tensor_2d(ctx0, BARK_GGML_TYPE_F32, n_embd, N));
        }

        // projection
        {
            cur = bark_ggml_mul_mat(ctx0, model->layers[il].c_attn_proj_w, cur);

            if (bias) {
                cur = bark_ggml_add(ctx0, cur, model->layers[il].c_attn_proj_b);
            }
        }

        // add the input
        cur = bark_ggml_add(ctx0, cur, inpL);

        struct bark_ggml_tensor* inpFF = cur;

        // feed-forward network
        {
            // norm
            {
                cur = bark_ggml_norm(ctx0, inpFF, EPS_NORM);

                // cur = ln_2_g*cur + ln_2_b
                cur = bark_ggml_mul(ctx0, cur, model->layers[il].ln_2_g);

                if (bias) {
                    cur = bark_ggml_add(ctx0, cur, model->layers[il].ln_2_b);
                }
            }

            // cur = fc_w*cur + fc_b
            cur = bark_ggml_mul_mat(ctx0, model->layers[il].c_mlp_fc_w, cur);

            if (bias) {
                cur = bark_ggml_add(ctx0, cur, model->layers[il].c_mlp_fc_b);
            }

            cur = bark_ggml_gelu(ctx0, cur);

            // projection
            cur = bark_ggml_mul_mat(ctx0, model->layers[il].c_mlp_proj_w, cur);

            if (bias) {
                cur = bark_ggml_add(ctx0, cur, model->layers[il].c_mlp_proj_b);
            }
        }

        // input for next layer
        inpL = bark_ggml_add(ctx0, cur, inpFF);
    }

    // norm
    {
        inpL = bark_ggml_norm(ctx0, inpL, EPS_NORM);

        // inpL = ln_f_g*inpL + ln_f_b
        inpL = bark_ggml_mul(ctx0, inpL, model->ln_f_g);

        if (bias) {
            inpL = bark_ggml_add(ctx0, inpL, model->ln_f_b);
        }
    }

    inpL = bark_ggml_mul_mat(ctx0,
                        model->lm_heads[0],
                        bark_ggml_view_1d(ctx0, inpL, inpL->ne[0], (inpL->ne[1] - 1) * inpL->nb[1]));

    bark_ggml_build_forward_expand(gf, inpL);
...
static bark_ggml_cgraph* bark_build_fine_gpt_graph(
    gpt_model* model,
    bark_ggml_allocr* allocr,
    bark_sequence& tokens,
    int codebook_idx,
    ...
    struct bark_ggml_tensor* input = bark_ggml_new_tensor_2d(ctx0, BARK_GGML_TYPE_I32, N, n_channels);
    bark_ggml_allocr_alloc(allocr, input);

    struct bark_ggml_tensor* tok_emb = bark_ggml_new_tensor_2d(ctx0, BARK_GGML_TYPE_F32, n_embd, N);
    bark_ggml_allocr_alloc(allocr, tok_emb);

    if (!bark_ggml_allocr_is_measure(allocr)) {
        bark_ggml_backend_tensor_set(input, tokens.data(), 0, N * n_channels * bark_ggml_element_size(input));
        bark_ggml_set_zero(tok_emb);
    }

    for (int wte_ix = 0; wte_ix < codebook_idx + 1; wte_ix++) {
        struct bark_ggml_tensor* cur = bark_ggml_get_rows(ctx0,
                                                model->wtes[wte_ix],
                                                bark_ggml_view_1d(ctx0, input, N, wte_ix * input->nb[1]));

        tok_emb = bark_ggml_add(ctx0, tok_emb, cur);
    }
    bark_ggml_set_name(tok_emb, "tok_emb");

    struct bark_ggml_tensor* position = bark_ggml_new_tensor_1d(ctx0, BARK_GGML_TYPE_I32, N);
    bark_ggml_allocr_alloc(allocr, position);
    if (!bark_ggml_allocr_is_measure(allocr)) {
        for (int32_t i = 0; i < N; ++i) {
            bark_ggml_backend_tensor_set(position, &i, i * sizeof(int32_t), sizeof(i));
        }
    }
    bark_ggml_set_name(position, "position");

    struct bark_ggml_tensor* pos_emb = bark_ggml_get_rows(ctx0, model->wpe, position);
    bark_ggml_set_name(pos_emb, "pos_emb");
    ...
    // wte + wpe
    struct bark_ggml_tensor* inpL = bark_ggml_add(ctx0, tok_emb, pos_emb);
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        {
            // cur = attn_w*cur
            cur = bark_ggml_mul_mat(ctx0, model->layers[il].c_attn_attn_w, cur);
            ...
            struct bark_ggml_tensor* KQ_soft_max = bark_ggml_soft_max_inplace(ctx0, KQ_scaled);
    ...
    // inpL = WTE * inpL
    struct bark_ggml_tensor* lm_head = model->lm_heads[codebook_idx - n_codes_given];
    inpL = bark_ggml_mul_mat(ctx0, lm_head, inpL);
...
static bool bark_eval_encoder_internal(
    ...
    const int64_t t_predict_us_start = bark_ggml_time_us();

    // reset the allocator to free all the memory allocated during the previous inference
    bark_ggml_allocr_reset(allocr);

    struct bark_ggml_cgraph* gf = bark_build_gpt_graph(
    ...
    struct bark_ggml_tensor* inpL = gf->nodes[gf->n_nodes - 1];
    ...
    bark_ggml_backend_tensor_get(inpL, logits.data(), 0, sizeof(float) * n_vocab);
    ...
    model.t_predict_us += bark_ggml_time_us() - t_predict_us_start;
...
static bool bark_eval_text_encoder(struct bark_context* bctx, int n_threads) {
    ...
    for (int i = 0; i < n_steps_text_encoder; i++) {
        ...
        if (!bark_eval_encoder_internal(model, allocr, input, logits, &n_past, true, n_threads)) {
            LOGE("%s: Could not generate token\n", __func__);
            return false;
        }
...
bool bark_forward_text_encoder(struct bark_context* bctx, int n_threads) {
    const int64_t t_main_start_us = bark_ggml_time_us();
    ...
    // allocate the compute buffer
    {
        ...
        size_t align = bark_ggml_backend_get_alignment(model.backend);
        bctx->allocr = bark_ggml_allocr_new_measure(align);
        ...
        size_t mem_size = bark_ggml_allocr_alloc_graph(bctx->allocr, gf);

        // recreate the allocator with the required memory
        bark_ggml_allocr_free(bctx->allocr);
        bctx->buf_compute = bark_ggml_backend_alloc_buffer(model.backend, mem_size);
        bctx->allocr = bark_ggml_allocr_new_from_buffer(bctx->buf_compute);
    ...
    if (!bark_eval_text_encoder(bctx, n_threads)) {
        LOGE("%s: failed to forward text encoder\n", __func__);
        return false;
    }

    model.t_main_us = bark_ggml_time_us() - t_main_start_us;
    bctx->stats.t_semantic_us = model.t_main_us;

    bark_print_statistics(&model);

    bark_ggml_backend_buffer_free(bctx->buf_compute);
    bark_ggml_allocr_free(bctx->allocr);
...
bool bark_forward_coarse_encoder(struct bark_context* bctx, int n_threads) {
    ...
    if (!bark_eval_coarse_encoder(bctx, n_threads)) {
        LOGE("%s: failed to forward coarse encoder\n", __func__);
        return false;
    }
...
static bool bark_eval_fine_encoder_internal(
    ...
    struct bark_ggml_cgraph* gf = bark_build_fine_gpt_graph(
    ...
    bark_ggml_backend_tensor_get(inpL, logits.data(), 0, sizeof(float) * n_vocab * block_size);
...
static bool bark_forward_eval(struct bark_context* bctx, int n_threads) {
    ...
    if (!bark_forward_fine_encoder(bctx, n_threads)) {
        LOGE("%s: failed to forward fine encoder\n", __func__);
        return false;
    }
...
bool bark_generate_audio(struct bark_context* bctx, const char* text, int n_threads) {
    ...
    if (!encodec_decompress_audio(bctx->encodec_ctx, encodec_tokens.data(), encodec_tokens.size(), n_threads)) {
        LOGI("%s: Could not generate waveform from tokens with Encodec\n", __func__);
        return false;
    }
    ...
    bctx->stats.t_eval_us = bark_ggml_time_us() - t_start_eval_us;
...
static void bark_free_model(struct gpt_model* model) {
    ...
    if (model->ctx) {
        bark_ggml_free(model->ctx);
    }

    bark_ggml_backend_buffer_free(model->buffer_w);
    bark_ggml_backend_free(model->backend);
}
...
bool bark_model_weights_quantize(std::ifstream& fin, std::ofstream& fout, bark_ggml_ftype ftype) {
    gpt_model model;
    gpt_hparams hparams;
    ...
    // load hparams
    {
        ...
        const int32_t qntvr_src = hparams.ftype / BARK_GGML_QNT_VERSION_FACTOR;
        int32_t ftype_dst = BARK_GGML_QNT_VERSION * BARK_GGML_QNT_VERSION_FACTOR + ftype;
        ...
        LOGI("%s: qntvr (src) = %d\n", __func__, qntvr_src);
        LOGI("%s: ftype (dst) = %d\n", __func__, ftype_dst);
        LOGI("%s: qntvr (dst) = %d\n", __func__, BARK_GGML_QNT_VERSION);
    ...
    if (!bark_ggml_quantize_weights(fin, fout, ftype, to_quant, {})) {
        LOGE("%s: failed to quantize model\n", __func__);
        return false;
    }
...
bool bark_model_quantize(const char* fname_inp, const char* fname_out, enum bark_ggml_ftype ftype) {
    LOGI("%s: loading model from '%s'\n", __func__, fname_inp);
    ...
    if (!fin) {
        LOGE("%s: failed to open '%s' for reading\n", __func__, fname_inp);
        return false;
    }

    auto fout = std::ofstream(fname_out_str, std::ios::binary);
    if (!fout) {
        LOGE("%s: failed to open '%s' for writing\n", __func__, fname_out);
        return false;
    }
    ...
    if (!bark_model_weights_quantize(fin, fout, ftype)) {
        LOGE("%s: failed to quantize text model\n", __func__);
        return false;
    }

    // coarse model
    if (!bark_model_weights_quantize(fin, fout, ftype)) {
        LOGE("%s: failed to quantize coarse model\n", __func__);
        return false;
    }

    // fine model
    if (!bark_model_weights_quantize(fin, fout, ftype)) {
        LOGE("%s: failed to quantize fine model\n", __func__);
        return false;
    }
