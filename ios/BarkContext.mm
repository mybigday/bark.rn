...
- (NSDictionary *)generate:(NSString *)text out_path:(NSString *)out_path {
    ...
    try {
        ...
        NSLog(@"Generating %s with %d threads", c_text, n_threads);
        success = bark_generate_audio(context.get(), c_text, n_threads);
    } catch (const std::exception &e) {
        @throw [NSException exceptionWithName:@"BarkContext" reason:[NSString stringWithUTF8String:e.what()] userInfo:nil];
    }
    if (success) {
        int audio_samples = bark_get_audio_data_size(context.get());
        float *audio_data = bark_get_audio_data(context.get());
        barkrn::pcmToWav(audio_data, audio_samples, sample_rate, [out_path UTF8String]);
    }
    return @{
        @"success": @(success),
